    ...
    <form method="POST" action="{{ route('admin.settings.update') }}" class="admin-grid-form">
        ...
        @foreach([
            @foreach([
                ...
                'footer_social_kicker' => 'Footer Social Kicker',
                'footer_social_title' => 'Footer Social Title',
                'footer_social_text' => 'Footer Social Text',
            ] as $key => $label)
        ] as $key => $label)
        ...
        <label class="span-2">
            Footer Social Text
            <textarea name="footer_social_text">{{ old('footer_social_text', $settings['footer_social_text'] ?? '') }}</textarea>
        </label>
